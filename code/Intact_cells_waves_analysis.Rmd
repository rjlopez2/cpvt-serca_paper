...
```
>NOTE:this two functions should go to the package `extract_model_info` and `plot_residuals` -> donde
...
wave_kinetics_df_nest <- wave_kinetics_df_nest %>%
  mutate(superplots = map2(data, variables, ~ {.x %>% superplot_func(yaxe = "value", 
                                                                     ...
                                                                     scaled_to = "width",
                                                                     adjusted = 1.5) + 
      ggplot2::ylab(.y)}))  # fix the label for y axis
...
```
```{r, results='show', include=TRUE, collapse = TRUE, fig.width=12, fig.asp = 0.3, out.width='100%'}
wave_freq_df_nest$superplots %>% 
  plot_grid(plotlist = ., ncol = 3)
```

NOTE: make occurence and cumulatie occurence plot

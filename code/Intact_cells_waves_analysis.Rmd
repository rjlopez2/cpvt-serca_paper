...
Note: Luckily for this set of the experiment you can save the entire folder withouth cherry-picking specific files/subfolders. But for other datasets you may need to check.
...
# model_raw_nsdf
```
>NOTE:this two functions should go to the package `extract_model_info` and `plot_residuals` -> Done
...
first we design a matrix to make the mdesired comparisons
...
emmens_ns_df %>% 
  ...
  filter(!str_detect(model_name, "no")) %>% # selecting the model with interaction
  # write_csv()
  write_csv(paste(output_results_dir$kinetics, 
                  paste(format(Sys.time(),
                               '%Y%m%d'),
...
wave_kinetics_df_nest <- wave_kinetics_df_nest %>%
  mutate(superplots = map2(data, variables, ~ {.x %>% superplot_func(yaxe = "value", 
                                                                     base_violin = "cells",
                                                                     dot_layer = "cells",
                                                                     animal_layer = T,
                                                                     wave_alpha = 0.1,
...
wave_kinetics_df_nest$superplots %>% 
  plot_grid(plotlist = .,
    ...
    #       wave_kinetics_df_nest$superplots,
    #       ~ ggsave(plot = .y, filename = paste(here(),
    #                                            "figures",
    #                                            "intact_cells",
    #                                            "kinetics", paste(format(Sys.Date(),
    #                                                                     "%y%m%d"),
    #                                                              paste0(.x,".pdf"),
    #                                                              sep = "_"),
    #                                            sep = "/"),
...
export results only for presentation
...
wave_freq_df_nest %>% 
  ...
  plot_grid(plotlist = .,
            ncol = 3)
   ########## Save your plot ########## 
    # walk2(list("superplots", "cum_occu_plot", "occu_bar_plot"),
    #       ~ ggsave(plot = .x, filename = paste(here(),
    #                                            "figures",
    #                                            "intact_cells",
    #                                            "latency", paste(format(Sys.Date(),
    #                                                                     "%y%m%d"),
    #                                                              paste0(.y,".pdf"),
    #                                                              sep = "_"),
    #                                            sep = "/"),
...
- 2. [x] Hypothesis testing (make mix model) and export results 

- 3. [x] Standard Plot and export figures 

...
we create first the marginal means object and then we design a matrix to make the desired comparisons. Here we want to compare genotypes differences among control and or drug application under different drugs, so we design the matrix accordingly
...
```{r results='show', include=TRUE}
coef(pairs(emmens_ns_df[, "em_means"][[1]][[1]]))
```

coefficient matrix in the interaction model

```{r results='show', include=TRUE}
coef(pairs(emmens_ns_df[, "em_means"][[1]][[2]]))
```



and here is our final matrix 

```{r results="show", incude = TRUE}
contrast_matrix <- list("WT_Control_Fab vs CPVT_Control_Fab" = 
                          c(1, rep(0, 11)) - c(0, 1, rep(0, 10)),
                          
                        "WT_Fab_Fab vs CPVT_Fab_Fab" =
                          c(rep(0, 8), 1, rep(0, 3)) - c(rep(0, 9), 1, rep(0, 1)),
...
----->>>>> you stay here. 17.07.2022. TODO: fix path to export and name of variables to display in the next code chunks

Here we export the test contrast from the wave kinetics parameters: `r wave_kinet_parameters` and can be found in the directory: `r output_results_dir$kinetics`
...
```{r, eval=params$output_results, results='show', include=TRUE, collapse = TRUE, fig.width=12, fig.asp = 0.3, out.width='200%'}
